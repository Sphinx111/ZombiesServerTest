    ...
    if (mouseButton == LEFT) {
      if (mouseX > uiLayer.uiPanelX && mouseY < uiLayer.uiPanelY + uiLayer.uiPanelHeight || (mouseX > uiLayer.uiActiveXMin && mouseX < uiLayer.uiActiveXMax && mouseY > uiLayer.uiActiveYMin && mouseY < uiLayer.uiActiveYMax)) {
        ...
        if (creatorMode) {
          ...
          } else if (currentType == BlockType.ACTOR) {
            //actor creation mode
          } else if (currentType == BlockType.ITEM) {
            //item creation mode
            Vec2 mousePos = new Vec2(newMouseX, newMouseY);
            itemClickPos = box2d.coordPixelsToWorld(mousePos);
            mapHandler.createItemByMouse(itemClickPos, ItemType.ITEM_RIFLE);
            itemClickPos = null;
          }
        } else {
          holdClick = true;
